...
class TestAdaptiveGaussHermite:
    ...
    def _dense(eta, sign, inv_scale, tau_sq, n=200001):
        ...
        m = g.max()
        return m + np.log(np.trapz(np.exp(g - m), b))
...
class TestSharedLayerSurface:
    ...
    def test_metadata_routing_surface(self):
        ...
        assert m.set_fit_request() is m          # stub returns self
...
class TestMetadataRequestStubsWarn:
    """The ``set_*_request`` stubs are honest about being stubs.

    Returning ``self`` silently would let a caller believe routing had been
    configured when nothing had been recorded, and metadata routing failing
    open is how a sample weight quietly stops being applied.
    """

    def test_set_predict_request_warns_when_given_kwargs(self):
        from MissLearn import MissLinear
        m = MissLinear(compute_se=False)
        with pytest.warns(UserWarning, match='no-op stub'):
            out = m.set_predict_request(sample_weight=True)
        assert out is m, 'the stub must still chain'

    def test_set_predict_request_is_silent_with_no_kwargs(self):
        from MissLearn import MissLinear
        m = MissLinear(compute_se=False)
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter('always')
            out = m.set_predict_request()
        assert out is m
        assert not [w for w in caught if 'no-op stub' in str(w.message)]
...
def _all_estimators():
    """Every fittable estimator in the public namespace, by name."""
