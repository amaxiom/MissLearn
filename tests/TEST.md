...
The first three are the correctness suites and all three must be green: 2,770 tests, comprising 1,712 unit, 1,042 conformance and 16 property, of which 57 skip and 1 is a declared xfail. Coverage is over **95%** of statement-and-branch units. The skips are conditional on an optional package or on an estimator not offering the method under test; none is a disabled check.

No wall-clock time is quoted for the suites, deliberately. Every figure measured for them on this project has been taken on a machine doing something else, and the spread across such runs has been several-fold on identical work. A run that feels slow is far more likely to be sharing cores than to be telling you something about the code, so check what else is running first. If a figure is needed, measure it with nothing else active and record what was running when you did.

The last two suites are measurement rather than pass/fail and CI does not gate on them.
